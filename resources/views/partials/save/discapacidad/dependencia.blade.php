    ...
    <div class='col-md-12 form-group'>
        <select style="width:100%;" name="tipo_dependencia" class='form-control permanente capitalize select-tag' id='inputSelect'>
            @foreach($dependencias as $dependencia)
	            <option
	                @if((@old('tipo_dependencia') == $dependencia->id) || (@old('tipo_dependencia') == null && isset($persona) && $dependencia->id == $persona->ficha_beneficiario->ficha_discapacidad->tipo_dependencia_id))
	                    selected
	                @endif
